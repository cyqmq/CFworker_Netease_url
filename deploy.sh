...
ID=$($W kv namespace list --json 2>/dev/null | node -e '
...
if [ -z "$ID" ]; then
  echo "    未找到，创建新命名空间 ..."
  ID=$($W kv namespace create "$NS_TITLE" --json 2>/dev/null | node -e '
    let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{try{console.log(JSON.parse(s).id)}catch(e){}});
  ')
fi
...
if [ -n "$ID" ]; then
  ...
  echo "    ⚠️ 未能获取/创建 KV 命名空间（可能权限/网络问题），将不使用 KV 部署。"
  echo "==> 部署（不含 KV，配置：wrangler.toml）..."
  $W deploy
fi

# 首次部署：为缺失的 KV 键写占位（容错，不影响部署结果）
if [ -n "$ID" ]; then
  echo "==> 检查 KV 配置占位 ..."
